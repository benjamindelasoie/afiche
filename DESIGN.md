...
| 2026-07-26 | `PageShell` chrome/content width split — shared chrome renders at a consistent width; the content column varies per page | The `/cartelera` masthead read narrower than the homepage's because its container was `max-w-5xl` vs the homepage `max-w-6xl` — users noticed it as "different headers". Widening the cartelera *content* to 6xl (a first attempt) contradicted the documented 2026-06-06/07 decision that `/cartelera`'s reading column stays `max-w-5xl`. Resolved with a `PageShell` layout primitive: the shared `Masthead` + `PageFooter` **chrome** now spans `max-w-6xl` on both `/` and `/cartelera` (so the header is identical across the front door and Ver-todo — the reported bug, fixed properly), while `/cartelera` wraps its day-list in a `ContentColumn` clamped to the documented 5xl. The masthead hairline rule therefore overhangs the narrower content by ~40px each side on `/cartelera` — the intentional "chrome frames content" editorial look (a newspaper nameplate spanning wider than its columns). `PageShell` is also the single home of the `w-full min-w-0` flex-item invariant (CLAUDE.md #1); `layout-invariants.test.ts` asserts it there once instead of re-checking every page. |
| 2026-07-26 | Shared UI primitive + design-token layer (`src/app/_components/ui/`) | Repeated layout scaffolding and style recipes — the `<main>` shell, the "Actualizado" footer, the "← Cartelera" back-link, the 404 shell, the carmine hover-rail row interaction, tag pills, mono micro-caps labels, section headings, the Próximamente index — were copy-pasted across 6 pages + 8 components and had drifted (the header bug above was one symptom). Extracted a presentational primitive layer that composes on top of the `@theme` primitive tokens (colors / tracking / fonts stay in globals.css): `cn()` (clsx + tailwind-merge) for predictable class overrides; **token components** `Caps` (eyebrow / card micro-caps), `Pill` (solid / ghost × sm / md tag chips), `SectionHeading` (display / bordered); **class-recipe helpers** `hoverRail({inset,gutter})` + `focusRing` (CVA-style — callers keep structural freedom over element / layout, the recipe owns the signature carmine hover-tick + de-tint); **layout primitives** `PageShell` / `ContentColumn` / `PageFooter` / `BackLink` / `NotFoundShell` / `StretchedLink`; plus the domain `UpcomingIndex` (merged the ~90 %-identical `/cartelera` + `/sala` Próximamente indexes into one `showCinema` / `lastScreeningPerFilm`-parametrized component) and `DayTimeChip`. Naming: PascalCase files matching the export, `ui/` for domain-agnostic primitives vs `_components/` for domain components. Rendering preserved (verified via /browse across every view type + the full invariant suite, 650 tests); `hoverRail` also standardizes the row-hover bg-fade transition that `FilmRow` had been missing. |
