...
The work came out of a QA pass on 2026-07-27 that started as a routine health check and turned up three stacked bugs behind a single symptom, each of which would have looked fixed while nothing changed.

### Added

- **Staleness alerting — `GET /api/health/freshness`.** A *failed* scrape was always loud; a scrape that **never ran** was silent, because no code executes to report it. That is the failure that actually occurred: between 2026-07-20 and 2026-07-27 the scrape box stayed asleep, afiche served week-old showtimes, and nothing signalled it. The check therefore lives **on Vercel, not on the scrape box** — the box being off is precisely what it detects. It reads the most recent successful `scrape_runs.finished_at` (the value the footer renders as "actualizado el …") and pings Telegram past a 26h threshold. Vercel cron calls it daily at 11:00 BA, two hours after the morning scrape should have landed. **Alerting is opt-in and dormant until configured** — an unset deploy answers 200 with `alerting: "disabled"` and the reason, rather than erroring daily for a feature nobody switched on. Setting `CRON_SECRET` both enforces the bearer token and unlocks alerting; while it is unset the endpoint is readable but structurally cannot send, which is what keeps it from being hammered into spamming a phone. Setup in `DEPLOY.md` §5.
- **Hide films from the cartelera** (`films.hidden_at` + Ocultar / Mostrar on `/admin/unmatched`). Some rows are not films and no matcher will ever fix them — festival umbrella programs ("SMOF, festival de cine de animación", the "CUADRO A CUADRO" series) and "PELÍCULA SORPRESA" mystery screenings. Visibility is enforced at the `fetchRows` choke point plus the four queries that bypass it, so hiding covers the cartelera, `/sala` agendas, `/pelicula` detail, search, the `.ics` route, JSON-LD **and** the MCP tools in one move. Hiding sets `skip_tmdb`, symmetrically reversed on unhide. Nothing is deleted; hidden rows stay listed and restorable.
- **`robots.txt` + `sitemap.xml`.** Both were missing — afiche is a discovery product whose ~500 `/pelicula` pages were reachable only by following links off a cartelera that shows just the current window, so anything not screening this week was effectively invisible to search. The sitemap is generated per request (the film set turns over with every scrape) and lists `/`, `/cartelera`, `/acerca`, every venue, and every film with a still-upcoming screening — excluding hidden films and films whose screenings have passed, since `/pelicula/<slug>` 404s in both cases and a sitemap full of 404s is worse than a smaller honest one. `robots.txt` points at it and disallows `/admin` and `/api`.
- **Editorial 404 at the root.** `src/app/not-found.tsx` was missing, so any mistyped or stale URL fell through to Next's built-in *"This page could not be found"* — English, on an es-AR site that already had an editorial 404 on `/sala` and `/pelicula`. Now routed through the same `NotFoundShell`.
- **Error and loading boundaries beyond `/sala`.** TODO #37 gave `/sala/[id]` an `error.tsx` + `loading.tsx` and stopped there; every other route fell through to framework defaults. Adds a root `error.tsx`, a `global-error.tsx` (inline-styled, since it replaces the root layout and cannot assume the Tailwind layer loaded), and a `loading.tsx` for `/pelicula/[slug]`.
- **Director-pivot rescue** (`searchMoviesByDirector`). When title search returns zero candidates — common when a venue uses an Argentine release title TMDB doesn't carry — the matcher now tries the other axis it holds. Deliberately narrow: accepts only when a scraped year exists **and** exactly one directed credit falls in the ±1 year window. Title similarity cannot gate this path; measured on prod, a correct rescue scored **0.495** while a wrong one scored **0.568**, so any similarity cutoff picks the wrong film. The discriminator is year uniqueness.

### Fixed

- **A matcher improvement can now reach its own backlog** (`films.match_attempt_version` + `MATCHER_VERSION`). v0.3.9.0's self-healing shipped and was a **no-op in production for a month**: a deterministic miss parks a row at `none-attempted`, and the only unlock fired when *scraped data* changed — but improving the matcher changes no data, so the rows the feature targeted were exactly the rows it could never touch. Rows are now stamped with the matcher version that failed them, and bumping the constant re-opens them automatically. Bounded by the existing future-screening guard, so a bump costs one TMDB call per still-showing stuck row.
- **Director verification was rejecting correct matches at confidence 1.000.** Two causes. Co-directors were split on commas only, so a Spanish conjunction left `"Juan Cabral y Santiago Franco"` as one blob against TMDB's two credits; now splits on `, / & + y e and` while keeping the unsplit form, so `"Ortega y Gasset"` still matches whole. And a surname-anchored tier handles given-name hypocorisms past Levenshtein's reach (TMDB credits Chaplin as "Charlie", venues scrape "Charles"). That tier uses a **curated alias table rather than a similarity threshold**, because the measured distributions overlap the wrong way — `juan/juana` 0.960 and `marco/marcos` 0.967 both outscore `charles/charlie` 0.943, so no cutoff separates hypocorisms from gender variants.
- **Three duplicate films collapsed** as a direct result: **El Partido**, **Plesiosaurios vivos** and **Gioia Mía** were each appearing twice with their screenings split across two pages. Cross-venue consolidation is the product's core promise, so these mattered more than the match count. Active unmatched films with upcoming screenings: **20 → 15**.
- **The scrape box no longer drifts behind shipped code.** The scrape and TMDB pipeline runs on a home server, not on Vercel, and nothing in the release process updated it — it was found pinned at v0.3.7.3 while main was v0.3.9.0, which is *why* the self-healing had never executed. `scrape-cron.sh` now fast-forwards main before every run, runs `npm ci` when the lockfile moved, and applies pending migrations before scraping. Failure posture is asymmetric on purpose: a failed pull notifies and scrapes anyway (stale code beats no data), a failed migration aborts (a schema/code mismatch is worse).
- **The 18:00 scrape was dead code.** `STALE_HOURS=12` against a 09:00/18:00 schedule meant the evening run skipped every single day while the plist advertised twice-daily. Now 8, so both fire and incidental wake-ups still coalesce.

### Notes

- Known gap, documented and pinned by a test rather than quietly tolerated: `directorsMatch` tier 2 (Levenshtein-1 on the full name) still accepts one-edit gender variants — "María González" matches "Mario González". Pre-existing, not introduced here.
- `inspect-unmatched`'s sanity panel reports **49 `auto` matches below 0.92 confidence** — untriaged wrong-match candidates. Given this project treats a wrong match as worse than a miss, they are the most interesting remaining lead.
